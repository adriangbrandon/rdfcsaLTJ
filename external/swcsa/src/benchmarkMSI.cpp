...
// file: associated to the graph-index
...
// optional inputs
	...
	// 1. READ QUERIES and STRUCTURES INITIALIZATION
	...
	if (!queries) {
		fprintf(stderr,"\n Unable to read queries. Aborting!\n"); 
		exit(0);
	} else {
	...
	/**/		case Q_S0  : { sprintf(operation,"sVV"); break;}

	/**/		case Q_P  :  { sprintf(operation,"VpV"); break;}
	/**/		case Q_P0  : { sprintf(operation,"VpV"); break;}

	/**/		case Q_O  :  { sprintf(operation,"VVo"); break;}
	/**/		case Q_O0  : { sprintf(operation,"VVo"); break;}
	/**/	}
	...
	/**/	}
		...
		switch(queryType) {
			...
			case Q_SP0: 
			case Q_SP1: 
			case Q_SP2 : { sprintf(msg,"SP\?"); break;}
			
			case Q_PO :
			case Q_PO0 :
			case Q_PO1 :
			case Q_PO2 : { sprintf(msg,"\?PO"); break;}

			case Q_SO :
			case Q_SO0 :
			case Q_SO1 :
			case Q_SO2 : { sprintf(msg,"S\?O"); break;}
			
			case Q_S   :
			case Q_S0  : { sprintf(msg,"S\?\?"); break;}

			case Q_P   :			
			case Q_P0  : { sprintf(msg,"\?P\?"); break;}

			case Q_O  :			
			case Q_O0  : { sprintf(msg,"\?\?O"); break;}
		}
	#endif	
	...
	uint results = 0;
	// MAX_RESULTS constant defined in "interface.h" file
	uint * res = (uint *) malloc (sizeof(uint) * (MAX_RESULTS * 3 +1));    
	if (!res) {
		perror("ERROR MALLOC");
		exit(0);
	}
        ...
        for (i = 0; i < nqueries; i++) {
			...
			switch(queryType) {
				...
				{ 	int j;
					...
					printf("\n [%4d]Querying %s: %u results for <%d,%d,%d> -->",i, msg, results, query.s, query.p, query.o);
					for (j=0;j<res[0];j++) {
