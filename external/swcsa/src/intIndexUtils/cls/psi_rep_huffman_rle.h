...
#ifndef _PSI_REP_HUFFMAN_RLE_H
#define	_PSI_REP_HUFFMAN_RLE_H
...
/** Huffman + Rle representation for Psi 
 *  It uses the options (#defines) above.  
 * 
 *  @author Antonio Fariña
 */
 
class psi_rep_huffman_rle : public psi_rep {
  
public:

	//psi_rep_huffman_rle(unsigned int *Psi, size_t psiSize);
	psi_rep_huffman_rle(unsigned int *Psi, size_t psiSize, size_t inioffset, unsigned int T, unsigned int nS);
	//psi_rep_plain_rle(unsigned int *Psi, size_t psiSize, size_t inioffset, unsigned int T, unsigned int nS); // { //, uint *psiTypes=NULL){

	virtual ~psi_rep_huffman_rle();
	...
	static psi_rep_huffman_rle * psi_load (char *filename);
