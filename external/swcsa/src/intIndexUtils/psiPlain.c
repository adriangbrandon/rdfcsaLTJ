...
#ifdef PSI_PLAIN_LOG_BASED	
	#ifdef R3H_WITHGAPS
	psiValue = bitread64(cPsi->samples,position*cPsi->sampleSize,cPsi->sampleSize) + cPsi->OFFSET;
	#else
...
//	size_t samplesPtr = 0;
//	for(index=0; index<compressedPsi.n; index++) {
//					bitwrite64(compressedPsi.samples,samplesPtr,compressedPsi.sampleSize, compressedPsi.psi[index] - OFFSET);
//					samplesPtr += compressedPsi.sampleSize;
//	}
//	/*CHECK it works*/
//			samplesPtr = 0;
//			for(index=0; index<compressedPsi.n; index++) {
//							/**/size_t tmp = bitread64(compressedPsi.samples,samplesPtr,compressedPsi.sampleSize)+OFFSET;			
