...
THuff createHuff (uint *freq, uint lim, uint sorted)
   ...
   { THuff H;
	...
	{ H.s.spos[tree[i].symb] = i;
     ...
     if (onlyOneSymbol){	H.total += ((size_t)freq[tree[0].symb]) * tree[0].h.depth;}
     else {
	     for (i=0;i<=lim;i++) {
	       H.total += ((size_t)freq[tree[i].symb])* tree[i].h.depth;
	       //fprintf(stderr,"\n         ****Huff.c.total[%d] = %d x %d =%zu",i,freq[tree[i].symb],tree[i].h.depth,H.total);
	    }
	}
     free (tree);
...
void saveHuff (THuff H, FILE *f)
   ...
   { uint *symb = (uint *)malloc((H.lim+1)*sizeof(uint));
	 ...
	 size_t fwrite_err;
     fwrite_err= fwrite (&H.max,sizeof(uint),1,f); 
     fwrite_err= fwrite (&H.lim,sizeof(uint),1,f); 
     fwrite_err= fwrite (&H.depth,sizeof(uint),1,f); 
     fwrite_err= fwrite (symb,sizeof(uint),H.lim+1,f); 
     fwrite_err= fwrite (H.num,sizeof(uint),H.depth+1,f); 
     free (symb);
   }
...
THuff loadHuff (FILE *f, int enc)   //enc (0/1)-> do you only  want to perform encoding ??
   ...
   { THuff H;
     ...
     int i,d,dold,dact;
     size_t fread_err;
     fread_err= fread (&H.max,sizeof(uint),1,f); 
     fread_err= fread (&H.lim,sizeof(uint),1,f); 
     fread_err= fread (&H.depth,sizeof(uint),1,f); 
     symb = (uint *) malloc ((H.lim+1)*sizeof(uint));
     fread_err= fread (symb,sizeof(uint),H.lim+1,f); 
     if (enc) 
          { H.s.spos = (uint *) malloc ((H.max+1)*sizeof(uint));
            for (i=0;i<=H.max;i++) H.s.spos[i] = ~0;
     ...
     fread_err= fread (H.num,sizeof(uint),H.depth+1,f); 
     if (!enc) 
...
void saveHuffAfterDecode (THuff H, FILE *f)     
   { size_t fwrite_err;
     ...
     fwrite_err= fwrite (&H.lim,sizeof(uint),1,f); 
     fwrite_err= fwrite (&H.depth,sizeof(uint),1,f); 
     fwrite_err= fwrite (H.s.symb,sizeof(uint),H.lim+1,f); 
     
     fwrite_err= fwrite (H.fst,sizeof(uint),H.depth+1,f); 
     fwrite_err= fwrite (H.num,sizeof(uint),H.depth+1,f); 
   }
...
void loadHuffAfterDecode2 (THuff *H, FILE *f, int enc)   //enc (0/1)-> do you only  want to perform encoding ??
   ...
   { 
	...
	size_t fread_err; 
     ...
     fread_err= fread (H->s.symb,sizeof(uint),H->lim+1,f); 
 
     H->fst = (uint *) malloc ((H->depth+1)*sizeof(uint));
     fread_err= fread (H->fst,sizeof(uint),H->depth+1,f);
