...
int dual_rdfcsaSPO_s (void *gindex, int S, unsigned int **buffer) {

	return s(gindex, S,buffer);
...
int dual_rdfcsaOPS_s (void *gindex, int S, unsigned int **buffer) {

	twcsa *g = (twcsa *)gindex;
	
	uint *res= *buffer; //no malloc performed here!!

	//printf("\n @@@@@@@@@@@@@@@@ call to dual_rdfcsaOPS_s (S)= (%d,\?,\?)\n",S);fflush (stdout);
...
uint dual_rdfcsaSPO_init_s (void *gindex, int S,  uint *left, uint *right) {
	
	//printf("\n @@@@@@@@@@@@@@@@ call to dual_rdfcsaSPO_init_s (%d,\?,\?)\n",S);fflush (stdout);
...
//Computes the range [left,right] where triple <S> is found.
//Returns the size of the range (right-left+1), or 0 if <S> is not found.
uint dual_rdfcsaOPS_init_s (void *gindex, int S,  uint *left, uint *right) {
	
	return dual_rdfcsaSPO_init_s(gindex,S,left,right);
}
...
void gr_resultsSort_opsS_SPO(uint *s, size_t n, uint nEntries) {
...
double start ,end;
	...
	*/
	...
	// now  B[0.. 3n-1] has the "n" different triplets.
	//freq[0..n-1] has the "n" frequencies of such triplets.
	printf("\n source triplets = %zu, different triplets <s,\?,\?> = %lu\n",size_buffer/3,n);
 ...
 start = getTime2();	
 	...
 	printf("\n now performing <s,\?,\?> (dual_rdfcsaSPO_s() && dual_rdfcsaOPS_s()) queries over existing triplets:\n");
 	int results, results1 ;	
 	long total_results=0;
	...
	//skips i=0;
 	...
 	for (i=1; i< n ; i++) {
		...
		//results1 = s_0(gindex, B[i*3], res2); 		
 		...
 		results  = dual_rdfcsaSPO_s(g , B[i*3], res);   
 		results1 = dual_rdfcsaOPS_s(g2, B[i*3], res2);
 		
 	
 		if ((results != f[i]) || (results != results1)) { 
 			printf("\n s- operation failed (i=%lu)\n", i);
 			printf("\n<s,\?,\?>= <%u, %u, %u>",B[i*3], B[i*3+2], B[i*3+1]);
 			exit(0);
 		}
 		if (results > 0) {
 			uint *res_s = (*res)+1;
			...
			if ( (res_s[x] != B[i*3])    ||
				...
				printf("\n\n Retrieved triplet (res_i=%d: failed: <s,\?,\?> = <%u,\?,\?> and should be  <%u,\?,\?>\n", 
				x,res_s[x],   B[i*3] );fflush (stdout);
...
start = getTime2();	
	...
	fflush(stdout);fflush(stderr);
	//now perform spo queries.
 	printf("\n now performing <s,\?,\?> (dual_rdfcsaSPO_s() && dual_rdfcsaOPS_s()) queries over rather unexisting triplets:\n");
 	int results, results1 ;	
 	long total_results=0;
