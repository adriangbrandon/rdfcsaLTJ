...
namespace cds_utils
{
	...
	inline int __setbit__sdarray(uint *B, const int i, const int x) {
		int j,l;
		j = i / D;
		l = i % D;
		if (x==0) B[j] &= (~(1<<(D-1-l)));
	...
	inline int __setbit2__sdarray(unsigned char *B, const int i, const int x) {
		int j,l;
		j = i / 8;
		l = i % 8;
		if (x==0) B[j] &= (~(1<<(8-1-l)));
	...
	inline int __setbits__sdarray(uint *B, const int i, const int d, const int x) {
		int j;
		for (j=0; j<d; j++) {
			__setbit__sdarray(B,i+j,(x>>(d-j-1))&1);
		}
		return x;
	}

	inline int __getbit__sdarray(const uint *B, const int i) {
		int j,l;
		j = i >> logD;
		l = i & (D-1);
		return (B[j] >> (D-1-l)) & 1;
	...
	inline int __getbit2__sdarray(const unsigned char *B, const int i) {
		int j,l;
		j = i >> 3;
		l = i & (8-1);
		return (B[j] >> (8-1-l)) & 1;
	...
	#if 1
	inline uint __getbits__sdarray(const uint *B, const int i1, const int d) {
		qword x,z;
		int i = i1;
		B += (i >> logD);
		i &= (D-1);
	...
	#endif
	#if 0
	inline uint __getbits__sdarray(uint *B, int i, int d) {
		uint j,x;
		x = 0;
		for (j=0; j<d; j++) {
			x <<= 1;
			x += __getbit__sdarray(B,i+j);
		}
		return x;
	}
	#endif

	static const unsigned int _popCount__sdarray[] = {
	...
	inline unsigned int __popCount__sdarray(const uint x) {
		uint r;
		r = x;
	...
	inline unsigned int __popCount8__sdarray(const uint x) {
		uint r;
		#if 1
		r = x;
		r = ((r & 0xaa)>>1) + (r & 0x55);
