...
BitSequenceDelta::BitSequenceDelta(){
    ...
    BitSequenceDelta::BitSequenceDelta(unsigned int* array, ulong n, unsigned int sampling){
        vector<uint> positions;
        uint prev = (uint)-1;
        for (uint i=0;i<n;i++) {
            if (bitget_lcds(array,i)) {
                positions.push_back(i - prev);
                prev = i;
            }
        }
        //if the last bit is not marked, then we mark the next to the last to
        //force operations like access to work fine.
        if (!bitget_lcds(array,n-1)){
            positions.push_back(n-prev);
        }
        build(n,&positions[0],positions.size(),sampling);

    }
    
	BitSequenceDelta::BitSequenceDelta(const BitString & bs, unsigned int sampling) {
		unsigned int* array; ulong n;
		array= bs.getData(); n= bs.getLength();
    ...
    /**
     * Array is a sequence of deltas (position_of_current_one - position_prev_one). Note position_prev_one
     * is intialized to -1, not to 0 since delta codes cannot represent a 0. That is why operations like
     * select1 returns the corresponding delta - 1.
     */
void BitSequenceDelta::build(ulong bitmap_length, unsigned int* array, ulong n, unsigned int sampling){
    this->n = n;
    this->sampling = sampling;
    this->ones = n;
    this->length = bitmap_length;
    ...
    for(unsigned int i=0;i<this->n; i++){
        unsigned int bits_array = bits_lcds(array[i]);
        this->total_size += 1 + 2*(bits_lcds(bits_array)-1) + (bits_array-1);
    }
...
void BitSequenceDelta::encodeDelta(unsigned int val, unsigned int* deltaCodes, unsigned int* pos){
    ...
    unsigned int k = bits_lcds(val);
    unsigned int p = bits_lcds(k);
