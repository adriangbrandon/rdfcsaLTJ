...
namespace cds_static
{
	...
	void BitSequenceRRR::build(const uint * bitseq, size_t len, uint sample_rate) {
		...
		C_field_bits = bits_lcds(BLOCK_SIZE); 
		C = new uint[uint_len(C_len,C_field_bits)];
		for(uint i=0;i<uint_len(C_len,C_field_bits);i++)
			C[i] = 0;
		O_bits_len = 0;
		for(uint i=0;i<C_len;i++) {
			uint value = popcount_lcds(get_var_field_lcds(bitseq,i*BLOCK_SIZE,min((uint)len-1,(i+1)*BLOCK_SIZE-1)));
			assert(value<=BLOCK_SIZE);
			set_field_lcds(C,C_field_bits,i,value);
			ones += value;
		...
		uint O_pos = 0;
		for(uint i=0;i<C_len;i++) {
			uint value = (ushort)get_var_field_lcds(bitseq,i*BLOCK_SIZE,min((uint)len-1,(i+1)*BLOCK_SIZE-1));
			set_var_field_lcds(O,O_pos,O_pos+E->get_log2binomial(BLOCK_SIZE,popcount_lcds(value))-1,E->compute_offset((ushort)value));
			O_pos += E->get_log2binomial(BLOCK_SIZE,popcount_lcds(value));
		}
		C_sampling = NULL;
		this->O_pos = NULL;
	...
	void BitSequenceRRR::create_sampling(uint sample_rate) {
		...
		C_sampling_field_bits = bits_lcds(ones);
		...
		for(uint i=0;i<C_len;i++) {
			if(i%sample_rate==0)
				set_field_lcds(C_sampling,C_sampling_field_bits,i/sample_rate,sum);
			sum += get_field_lcds(C,C_field_bits,i);
		}
		for(uint i=(C_len-1)/sample_rate+1;i<C_sampling_len;i++)
			set_field_lcds(C_sampling,C_sampling_field_bits,i,sum);

		// Sampling for O (table S) (Code separated from previous construction for readability)
		O_pos_len = C_len/sample_rate+1;
		O_pos_field_bits = bits_lcds(O_bits_len);
		...
		for(uint i=0;i<C_len;i++) {
			if(i%sample_rate==0)
				set_field_lcds(O_pos,O_pos_field_bits,i/sample_rate,pos);
			pos += E->get_log2binomial(BLOCK_SIZE,get_field_lcds(C,C_field_bits,i));
		}
	}
	...
	bool BitSequenceRRR::access(const size_t i) const
	{
		...
		size_t pos_O = get_field_lcds(O_pos,O_pos_field_bits,nearest_sampled_value);
		size_t pos = i/BLOCK_SIZE;
		assert(pos<=C_len);
		for(size_t k=nearest_sampled_value*sample_rate;k<pos;k++) {
			size_t aux = get_field_lcds(C,C_field_bits,k);
			pos_O += E->get_log2binomial(BLOCK_SIZE,aux);
		}
		size_t c = get_field_lcds(C,C_field_bits,pos);
		return ((1<<(i%BLOCK_SIZE))&E->short_bitmap(c,get_var_field_lcds(O,pos_O,pos_O+E->get_log2binomial(BLOCK_SIZE,c)-1)))!=0;
	}
	...
	bool BitSequenceRRR::access(const size_t i, size_t &r) const
	{
		...
		uint sum = get_field_lcds(C_sampling,C_sampling_field_bits,nearest_sampled_value);
		uint pos_O = get_field_lcds(O_pos,O_pos_field_bits,nearest_sampled_value);
		uint pos = i/BLOCK_SIZE;
		uint k=nearest_sampled_value*sample_rate;
		if(k%2==1 && k<pos) {
			uint aux = get_field_lcds(C,C_field_bits,k);
			sum += aux;
			pos_O += E->get_log2binomial(BLOCK_SIZE,aux);
			k++;
		}
		...
		while(k<(uint)max(0,(int)pos-1)) {
			assert(((*a)&mask)==get_field_lcds(C,C_field_bits,k));
			assert((*a)/16==get_field_lcds(C,C_field_bits,k+1));
			sum += ((*a)&mask)+(*a)/16;
		...
		size_t c = get_field_lcds(C,C_field_bits,pos);
		short v = E->short_bitmap(c,get_var_field_lcds(O,pos_O,pos_O+E->get_log2binomial(BLOCK_SIZE,c)-1));
		sum += popcount_lcds(((2<<(i%BLOCK_SIZE))-1) & v);
		r = sum;
	...
	size_t BitSequenceRRR::rank1(size_t i) const
	{
		...
		size_t c = get_field_lcds(C,C_field_bits,pos);
		sum += popcount_lcds(((2<<(i%BLOCK_SIZE))-1) & E->short_bitmap(c,get_var_field_lcds(O,pos_O,pos_O+E->get_log2binomial(BLOCK_SIZE,c)-1)));
		return sum;
	}
	...
	size_t BitSequenceRRR::select0(size_t i) const
	{
		...
		while(start<end-1) {
			med = (start+end)/2;
			acc = med*sample_rate*BLOCK_SIZE-get_field_lcds(C_sampling,C_sampling_field_bits,med);
			if(acc<i) {
				if(med==start) break;
				start=med;
			}
		...
		acc = get_field_lcds(C_sampling,C_sampling_field_bits,start);
		while(start<C_len-1 && acc+sample_rate*BLOCK_SIZE==get_field_lcds(C_sampling,C_sampling_field_bits,start+1)) {
			start++;
			acc +=sample_rate*BLOCK_SIZE;
		}
		acc = start*sample_rate*BLOCK_SIZE-acc;
		pos = (start)*sample_rate;
		size_t pos_O = get_field_lcds(O_pos,O_pos_field_bits,start);

		// Sequential search over C
		size_t s = 0;
		for(;pos<C_len;pos++) {
			s = get_field_lcds(C,C_field_bits,pos);
		...
		while(acc<i) {
			...
			size_t block = E->short_bitmap(s,get_var_field_lcds(O,pos_O,new_posO-1));
			pos_O = new_posO;
			new_posO = 0;
	...
	size_t BitSequenceRRR::select1(size_t i) const
	{
		...
		while(start<end-1) {
			med = (start+end)/2;
			acc = get_field_lcds(C_sampling,C_sampling_field_bits,med);
			if(acc<i) {
				if(med==start) break;
				start=med;
			}
		...
		acc = get_field_lcds(C_sampling,C_sampling_field_bits,start);
		while(start<C_len-1 && acc==get_field_lcds(C_sampling,C_sampling_field_bits,start+1)) start++;
		pos = (start)*sample_rate;
		size_t pos_O = get_field_lcds(O_pos,O_pos_field_bits,start);
		acc = get_field_lcds(C_sampling,C_sampling_field_bits,start);
	...
	size_t BitSequenceRRR::getSize() const
	{
		// if(!show_once) {
		// 	show_once=true;
		// 	cout << "This bitmap has " << length << " bits_lcds, ie ~" << length/8 << " bytes plain" << endl;
