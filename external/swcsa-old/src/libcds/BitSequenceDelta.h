...
{
	#define DEFAULT_SAMPLING_DELTA 64
	
    class BitSequenceDelta: public BitSequence
    {

    public:
            BitSequenceDelta(unsigned int* array, ulong n, unsigned int sampling = DEFAULT_SAMPLING_DELTA);
			BitSequenceDelta(const BitString & bs, unsigned int sampling =DEFAULT_SAMPLING_DELTA);
  ...
  //@          
        ...
        private:
            ...
            void build(ulong, unsigned int* array, ulong n, unsigned int sampling);
