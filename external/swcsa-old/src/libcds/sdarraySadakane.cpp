...
{
	static unsigned int __selecttbl__sdarray[8*256];
	static int built = 0;

	void make___selecttbl__sdarray(void) {
		if(built) return;
		built = 1;
		int i,x,r;
		uint buf[1];
		...
		for (x = 0; x < 256; x++) {
			__setbits__sdarray(buf,0,8,x);
			for (r=0; r<8; r++) __selecttbl__sdarray[(r<<8)+x] = -1;
			r = 0;
			for (i=0; i<8; i++) {
				if (__getbit__sdarray(buf,i)) {
					__selecttbl__sdarray[(r<<8)+x] = i;
					r++;
				}
			}
		}
	...
	int selectd2_construct(selectd2 *select, const int n, const unsigned char *buf) {
		...
		m = 0;
		for (i=0; i<n; i++) m += __getbit2__sdarray(buf,i);
		select->n = n;
		select->m = m;
		//printf("n=%d m=%d\n",n,m);
	...
	int selectd2_select(const selectd2 *select, const int i1, const int f) {
		...
		if (il < 0) {
			...
			if (f == 1) {
				rr = p & (8-1);
				r -= _popCount__sdarray[*q >> (8-1-rr)];
				//p = p - rr;

				while (1) {
					rr = _popCount__sdarray[*q];
					if (r + rr >= i) break;
					r += rr;
					//p += 8;
					q++;
				...
				p = (q - select->buf) << 3;
				p += __selecttbl__sdarray[((i-r-1)<<8)+(*q)];
			}
			else {
				rr = p & (8-1);
				r -= _popCount__sdarray[(*q ^ 0xff) >> (8-1-rr)];
				//p = p - rr;

				while (1) {
					rr = _popCount__sdarray[*q ^ 0xff];
					if (r + rr >= i) break;
					r += rr;
					//p += 8;
					q++;
				...
				p = (q - select->buf) << 3;
				p += __selecttbl__sdarray[((i-r-1)<<8)+(*q ^ 0xff)];
			}
		}
		return p;
	}
		...
		if (il < 0) {
			...
			if (f == 1) {
				...
				if ((i>>logL) == ((i+1)>>logL)) {
					...
					p2 = (q - select->buf) << 3;
					p2 += __selecttbl__sdarray[((i-r-1)<<8)+(*q)];
				}
				else {
					p2 = selectd2_select(select,i+2,f);
				}
				...
				if ((i>>logL) == ((i+1)>>logL)) {
					...
					p2 = (q - select->buf) << 3;
					p2 += __selecttbl__sdarray[((i-r-1)<<8)+(*q ^ 0xff)];
				}
				else {
					p2 = selectd2_select(select,i+2,f);
				}
	...
	int selects3_construct(selects3 *select, const int n, const uint *buf) {
		...
		m = 0;
		for (i=0; i<n; i++) m += __getbit__sdarray(buf,i);
		select->n = n;
		select->m = m;
		...
		for (i=0; i<m*2; i++) __setbit2__sdarray(buf2,i,0);

		m = 0;
		for (i=0; i<n; i++) {
			if (__getbit__sdarray(buf,i)) {
				__setbit2__sdarray(buf2,(i>>d)+m,1);
				__setbits__sdarray(low,m*d,d,i & ((1<<d)-1));
				m++;
			}
		}
		...
		for (i=0; i<m*2; i++) __setbit2__sdarray(buf2,i,1-__getbit2__sdarray(buf2,i));
	...
	int selects3_select(const selects3 *select, const int i) {
		...
		/*if(select->lasti==(uint)i-1) {
			while(!__getbit2__sdarray(select->sd1->buf,++select->lasts));
		}
		else {
		...
		x <<= d;
		x += __getbits__sdarray(select->low,(i-1)*d,d);
		return x;
	}
	...
	int selects3_selectnext(const selects3 *select, const int i) {
		...
		while (1) {
			...
			w = __getbits__sdarray(q,x*d,d);
			if (w >= j) {
				if (w == j) {
					if(__getbit2__sdarray(select->hi,(8*y+r))) k2++;
					x++;
					r++;
				}
				break;
		...
		for(int kk=0;kk<8-r;kk++) {
			if(__getbit2__sdarray(select->hi,c)) {
				fin=1;
				break;
			}
			c++;
		...
		if(!fin) {
			...
			while(!__getbit2__sdarray(select->hi,c)) c++;
		}
		c -= (k2);
		return __getbits__sdarray(q,x*d,d)+((c)<<d);
	}
