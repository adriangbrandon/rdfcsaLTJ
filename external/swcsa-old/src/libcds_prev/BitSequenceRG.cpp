...
namespace cds_static
{
	...
	size_t BitSequenceRG::rank1(const size_t i1) const
	{
		...
		for (uint a=aux;a<i/Wlcds;a++)
			resp+=popcount8_lcds(data[a]);
		resp+=popcount8_lcds(data[i/Wlcds]  & ((1<<(i & mask31lcds))-1));
		return resp;
	}
	...
	size_t BitSequenceRG::select1(const size_t x1) const
	{
		...
		// then sequential search using popcount8_lcds over a int
		...
		//sequential search using popcount8_lcds over a int
		uint left;
		left=mid*factor;
		x-=rankmid;
		uint j=data[left];
		uint ones = popcount8_lcds(j);
		while (ones < x) {
			x-=ones;left++;
			if (left > integers) return n;
			j = data[left];
			ones = popcount8_lcds(j);
		}
	...
	size_t BitSequenceRG::select0(const size_t x1) const
	{
		...
		x-=rankmid;
		uint j=data[left];
		uint zeros = Wlcds-popcount8_lcds(j);
		while (zeros < x) {
			x-=zeros;left++;
			if (left > integers) return n;
			j = data[left];
			zeros = Wlcds-popcount8_lcds(j);
		}
