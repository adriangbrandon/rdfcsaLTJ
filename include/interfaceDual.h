...
int build_index_dual (const char * filename, char *build_options, void **index) ;    //<--- Será así: Adrián Dixit !!


int save_index_dual (void *index, const char *filename);
int load_index_dual (const char *filename, void **index);
int free_index_dual (void *index); //TODO, non tera que ser **index
...
bool is_last_level(void *iterator);
// devolver true cando hai duas variables fixadas, noutro caso false

void get_all(void *iterator, int type, uint* results);
